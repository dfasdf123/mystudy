...
public class Ex30webPhpMain extends AppCompatActivity {

    LinearLayout frame_member_list, frame_member_edit, frame_member_del;
    ////////////////////회원목록
    static ArrayList<Ex30ItemData> dataList = new ArrayList<>();
    static Ex30RecyclerAdapter adapter = new Ex30RecyclerAdapter(dataList);
    ...
    @Override
        ...
        setContentView(R.layout.activity_ex30_main_layout);
    ...
    void dataLoad()
    {
            ...
            public void onResponse(String response) {
                try {
                    ...
                    for(int i=0; i< jArr.length();i++)
                    {
                        ...
                        //어댑터에 하나씩 추가
                        dataList.add(new Ex30ItemData(id[i], name[i],hp[i]));
                    }
                    //리사이클러뷰에 최종추가
        ...
        Ex30ServerRequestMemberList getUserCharLevelStat = new Ex30ServerRequestMemberList( responseListener);
    ...
    void delFind(String findId)
    {
        ...
        Ex30ServerRequestEditFind getUserCharLevelStat = new Ex30ServerRequestEditFind(findId, responseListener);
        RequestQueue queue = Volley.newRequestQueue(context);
        queue.add(getUserCharLevelStat);
    }
    void del()
    {
        ...
        Ex30ServerRequestDel del = new Ex30ServerRequestDel(delId, responseListener);
        RequestQueue queue = Volley.newRequestQueue(context);
        queue.add(del);
    }
    ...
    void edit()
    {
        ...
        Ex30ServerRequestEdit getUserCharLevelStat = new Ex30ServerRequestEdit(editId,editName,editHp, responseListener);
        RequestQueue queue = Volley.newRequestQueue(context);
        queue.add(getUserCharLevelStat);
    }
    ...
    void dlgShow()
    {
        Dialog dlgFinish = new Dialog(Ex30webPhpMain.this);
